...
const App = () => {
    ...
    useEffect(() => {
        ...
        const wait$ = waitTmp$.pipe(buffer(buff$), map(queue => queue.length), filter(clicks => clicks === 2));

// can`t resolve problem with button RESET doesn`t reset state value and restarts timer

        start$.subscribe(() => {
            seconds$.pipe(takeUntil(start$),takeUntil(reset$), takeUntil(stop$),takeUntil(wait$)).subscribe(() => {
                setTimeState({...tmpTime})
                switch (true) {

                    case tmpTime.h === 10 && tmpTime.m === 0 && tmpTime.s === 0:
